...
def initialize_browser(driver='/Users/pkrat/PycharmProjects/VORTEX-master/chromedriver'):
...
def scrape_urls(page_source):
    ...
    urls = ["https://www.linkedin.com/in/" + publicIdentifier[20:-1] for publicIdentifier in publicIdentifiers if
            publicIdentifier[20:-1] != "UNKNOWN"]
    return urls
...
def scrape_all_pages(total_profiles_encountered, num_results, page_source, num_pages, driver, linkedin_search_url,
                     total_profiles_scraped, total_profiles_scraped_history, all_profile_urls, country_code,
                     sub_region_code, geo_idx, current_time, industry_code="NA", industry_idx="NA", name="NA",
                     name_idx="NA"):
    ...
    #all_profile_urls.append({"Country_Code": country_code, "Sub_Region_Code": sub_region_code, "geo_index": geo_idx,
    #                        "Industry_Code": industry_code, "industry_index": industry_idx, "Name": name,
    #                        "name_indxex": name_idx, "Profile_urls": scraped_urls})
...
last_partial_result = max(
    [int(partial_result.split("-")[1].split(".")[0]) for partial_result in os.listdir("partial_results/") if
     partial_result[0] == "p"])
...
while True:
    ...
    # Break if no profiles are being scraped (something unknown is wrong)
        ...
        if total_profiles_scraped == total_profiles_scraped_history[-1] == total_profiles_scraped_history[-2] == \
                total_profiles_scraped_history[-3] == total_profiles_scraped_history[-4] == \
                total_profiles_scraped_history[-5]:
        ...
        for geo_idx, geo_code in enumerate(linkedin_geo_codes):
            ...
            # If results, start scraping
                ...
                # If there are results, scrape them
                    ...
                    if num_pages <= 100:
                        total_profiles_encountered, total_profiles_scraped, total_profiles_scraped_history, all_profile_urls, current_time = scrape_all_pages(
                            total_profiles_encountered, num_results, page_source, num_pages, driver,
                            linkedin_search_url, total_profiles_scraped, total_profiles_scraped_history,
                            all_profile_urls, country_code, sub_region_code, geo_idx, current_time)
                    ...
                    # If there are more than 100 pages, apply the industry filters
                        ...
                        for industry_idx, industry_code in enumerate(linkedin_industry_codes):
                            ...
                            print("Starting Scraping of Industry Code: {} out of {}".format(industry_idx + 1,
                                                                                            num_industry_codes))
                                ...
                                # If there are results, scrape them
                                    ...
                                    if num_pages <= 100:
                                        total_profiles_encountered, total_profiles_scraped, total_profiles_scraped_history, all_profile_urls, current_time = scrape_all_pages(
                                            ...
                                            linkedin_search_url, total_profiles_scraped, total_profiles_scraped_history,
                                            all_profile_urls, country_code, sub_region_code, geo_idx, current_time,
                                            industry_code=industry_code, industry_idx=industry_idx)
                                    ...
                                    # If there are more than 100 pages, apply the name filters
                                        ...
                                        for name_idx, name in enumerate(most_popular_names):
                                            ...
                                            # Print current name
                                            print("Starting Scraping of Name: {} out of {}".format(name_idx + 1,
                                                                                                   num_names))
                                            ...
                                            # If results, start scraping
                                                ...
                                                if not num_results:
                                                    ...
                                                    # Print that something is wrong
                                                    print("Something wrong, trying again, URL: {}".format(
                                                        driver.current_url))
                                                ...
                                                # If there are results, scrape them
                                                    ...
                                                    if num_pages <= 100:
                                                        total_profiles_encountered, total_profiles_scraped, total_profiles_scraped_history, all_profile_urls, current_time = scrape_all_pages(
                                                            total_profiles_encountered, num_results, page_source,
                                                            num_pages, driver, linkedin_search_url,
                                                            total_profiles_scraped, total_profiles_scraped_history,
                                                            all_profile_urls, country_code, sub_region_code, geo_idx,
                                                            current_time, industry_code=industry_code,
                                                            industry_idx=industry_idx, name=name, name_idx=name_idx)
